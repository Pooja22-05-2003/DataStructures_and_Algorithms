...
int solve(vector<int> arr1,vector<int> arr2,vector<int>arr3, int n){
    ...
    if(n>=2){
    DPa[2]=max(arr1[2]+DPa[2-1] , max(arr1[2]+DPb[2-2] , arr1[2]+DPc[2-2]));
    DPb[2]=max(arr2[2]+DPb[2-1] , max(arr2[2]+DPa[2-2] , arr2[2]+DPc[2-2]));
    DPc[2]=max(arr3[2]+DPc[2-1] , max(arr3[2]+DPb[2-2] , arr3[2]+DPa[2-2]));


    for(int i=3;i<=n;i++){
    DPa[i]=max(arr1[i]+DPa[i-1] , max(arr1[i]+DPb[i-2] , arr1[i]+DPc[i-2]));
    DPb[i]=max(arr2[i]+DPb[i-1] , max(arr2[i]+DPa[i-2] , arr2[i]+DPc[i-2]));
    DPc[i]=max(arr3[i]+DPc[i-1] , max(arr3[i]+DPb[i-2] , arr3[i]+DPa[i-2]));     
    }
  }

  cout<<"DPa[n]:"<<DPa[n]<<" DPb[n]:"<<DPb[n]<<" DPc[n]:"<<DPc[n]<<endl;
...
int main(){
    ...
    cout<<solve(arr1,arr2,arr3,n)<<endl;
	return 0;
}
