...
public:
    int solve(int ind,vector<int>&prices,bool buy,int fee){
        ...
        if(buy){
            int opt1=-prices[ind]-fee + solve(ind+1,prices,false,fee);
            int opt2= 0+solve(ind+1,prices,true,fee);
        ...
        if(!buy){
            ...
            int option1=0+solve(ind+1,prices,false,fee);
            int option2=prices[ind]+solve(ind+1,prices,true,fee);
